namespace ExBuddy.OrderBotTags.Fish
{
    ...
    public class FishTag : ExProfileBehavior
    {
        ...
        private async Task<bool> HandleBait()
        {
            ...
            if (!HasSpecifiedBait)
            {
                Log("You do not have the specified bait: " + Bait, Colors.Red);
                return isDone = true;
            }
            ...
            get
            {
                return
                    new Decorator(
                        ...
                        (Keepers.Count == 0 || Keepers.All(k => !string.Equals(k.Name, FishResult.FishName, StringComparison.InvariantCultureIgnoreCase)) || Keepers.Any(k => string.Equals(k.Name, FishResult.FishName, StringComparison.InvariantCultureIgnoreCase) && FishResult.ShouldMooch(k))),
                        new Sequence(
                            new Action(
                                r =>
                                {
        ...
        protected Composite ReleaseComposite
        {
            get
            {
                return
                    new Decorator(
                        ret =>
                        checkRelease && FishingManager.State == FishingState.PoleReady
                        && CanDoAbility(Abilities.Release) && Keepers.Count != 0,
                        new Sequence(
                            new Wait(
                                2,
                                ...
                                new Action(
                                    r =>
                                    {
                                        ...
                                        if (!Keepers.Any(FishResult.IsKeeper)
        ...
        {
            Logging.Write(color, string.Format("[Fish v{0}] {1}", Version, message));
        }
