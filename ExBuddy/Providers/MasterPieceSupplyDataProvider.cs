namespace ExBuddy.Providers
{
    ...
    public class MasterPieceSupplyDataProvider
    {
        ...
        static MasterPieceSupplyDataProvider()
        {
            ...
            {
                DataFilePath =
                    Directory.GetFiles(PluginManager.PluginDirectory, "*msd.xml", SearchOption.AllDirectories)
                        .FirstOrDefault();
            }
            ...
            var result =
                data.Root.Descendants("MS")
                    .FirstOrDefault(
                        e =>
                        e.Elements()
                            .Any(c => string.Equals(c.Value, itemName, StringComparison.InvariantCultureIgnoreCase)));
